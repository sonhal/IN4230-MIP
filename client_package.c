...
BYTE *ClientPackage_serialize(ClientPackage *package){
    ...
    BYTE *serialized_package = calloc(serialized_package_size, sizeof(BYTE));
    check_mem(serialized_package);

    uint16_t offset = sizeof(package->port);

    memcpy(serialized_package, &package->port, sizeof(package->port));
    memcpy(&serialized_package[offset], &package->destination, sizeof(package->destination));
    offset += sizeof(package->destination);
    memcpy(&serialized_package[offset], &package->data_size, sizeof(package->data_size));
    offset += sizeof(package->data_size);
    memcpy(&serialized_package[offset], package->data, package->data_size);
