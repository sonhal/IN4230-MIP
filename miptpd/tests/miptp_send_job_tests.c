...
char *test_MIPTPSendJob_create_destroy() {
    MIPTPSendJob *job = MIPTPSendJob_create(10, "Hello", strlen("Hello"), 1000);
...
char *test_MIPTPSendJob_next_package() {
    ...
    MIPTPSendJob *job = MIPTPSendJob_create(10, data, strlen(data), 1000);
...
char *test_MIPTPSendJob_receive_ack() {
    MIPTPSendJob *job = MIPTPSendJob_create(10, "Hello", strlen("Hello"), 1000);
    uint16_t seq_nr = 5;

    MIPTPSendJob_receive_ack(job, seq_nr);
...
char *test_MIPTPSendJob_is_finished() {
    ...
    MIPTPSendJob_receive_ack(job, 0);
    mu_assert(MIPTPSendJob_finished(job) == 0, "Wrong value, job is not finished");

    MIPTPSendJob_receive_ack(job, 1);
    mu_assert(MIPTPSendJob_finished(job) == 1, "Wrong value, job is finished");
...
char *all_tests(){
    ...
    mu_run_test(test_MIPTPSendJob_receive_ack);
